...
public static class SerilogConfigurator
{
	public static void Configure(IConfigurationRoot configuration)
	{
		...
		if (!Enum.TryParse(configuration["Serilog:MinimumLevel"], true, out LogEventLevel minimalLevel))
		{
		...
		Log.Logger = new LoggerConfiguration()
			...
			.WriteTo.File(logsPath, outputTemplate: fileOutputTemplate, rollingInterval: RollingInterval.Day)
			.CreateLogger();
	}
}
